...
import torch
from model.dqs import dqs
from model.lbs import lbs,batch_rodrigues
import pickle
...
class SMPLlayer(nn.Module):
    ...
    def forward(self, poses, shapes, Rh=None, Th=None, expression=None,
                ...
                only_shape=False, pose2rot=True,return_my_joints=True,verts_type="openpose_first",**kwargs):
        ...
        if return_verts or not self.use_joints:
            ...
            vertices, joints = self.lbs(shapes, poses, v_template,
                                        ...
                                        use_shape_blending=self.use_shape_blending, J_shaped=self.J_shaped)
            ...
            vertices, joints = self.lbs(shapes, poses, self.j_v_template,
                                        ...
                                        use_shape_blending=self.use_shape_blending, J_shaped=self.J_shaped)
            if return_my_joints:
                # print("verts_type",verts_type)
                if verts_type=="openpose_first":
                    my_joints_id = [32, 3, 6, 9, 25, 24, 40, 39, 29, 26, 30, 27, 31, 28, 22, 23, 36, 33, 37, 34, 38, 35,
                                    43,46]
                    vertices = vertices[:, my_joints_id, :]
                if verts_type=="smpl_first":
                    my_joints_id = [0, 3, 6, 9, 12, 15, 40, 39, 16, 17, 18, 19, 20, 21, 22, 23, 1, 2, 4, 5, 7, 8, 10,
                                11]
                    vertices = vertices[:, my_joints_id, :]
                if verts_type=="fusion":
                    my_joints_1= [32, 3, 6, 9, 25, 24, 40, 39, 29, 26, 30, 27, 31, 28, 22, 23, 36, 33, 37, 34, 38, 35,
                                    43,46]
                    my_joints_2= [0, 3, 6, 9, 12, 15, 40, 39, 16, 17, 18, 19, 20, 21, 22, 23, 1, 2, 4, 5, 7, 8, 10,
                                11]
                    vertices = (vertices[:, my_joints_1, :]+vertices[:, my_joints_2, :])/2
                if verts_type == None:
                    raise "the verts_type is None"
    ...
    def init_params(self, nFrames=1, nShapes=1, ret_tensor=False):
        params = {
            ...
            'Rh': np.repeat(np.array([[0,np.pi,0]]),nFrames,axis=0),
    ...
    def check_params(self, body_params):
        ...
        if body_params['poses'].shape[1] != self.NUM_POSES:
            body_params['poses'] = np.hstack(
                (body_params['poses'], np.zeros((nFrames, self.NUM_POSES - body_params['poses'].shape[1]))))
